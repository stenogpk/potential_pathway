...
import { ingestTextSource, isSupportedTextFile, ingestPdfSource, isPdfFile } from "./sourceIngestion.js";

export async function indexSource(source, file) {
  if (isPdfFile(file) || isPdfFile({ name: source?.fileName, type: source?.mimeType })) {
    const result = await ingestPdfSource({ source, file });
    ...
    return { source: { ...indexedSource, pageCount: result.pageCount }, chunks: result.chunks, chunkCount: result.chunkCount };
  }
