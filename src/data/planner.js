export function buildStudyPlan({ missionId, availableMinutes = 50, sessions = [], attempts = [], revisions = [], courseNodes = [] }) {
  ...
  if (topic && used2 < availableMinutes) plan.push({ type: "course", minutes: Math.min(availableMinutes - used2, 25), label: `Study next topic: ${topic.name}` });
