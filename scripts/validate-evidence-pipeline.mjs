...
const external = createExternalSourceRecord({ missionId: "pcs", title: "UPSC official page", url: "https://www.upsc.gov.in/examinations", publisher: "UPSC" });
if (getEvidenceLayer(userSource) !== "user-source") throw new Error("User source layer failed.");
if (getEvidenceLabel(userSource) !== "Provided PDF / source") throw new Error("User source label failed.");
...
const blockedExternal = createExternalSourceRecord({ missionId: "pcs", title: "Untrusted article", url: "https://example.com/article" });
...
if (evidence.chunkCount !== 1 || evidence.chunks[0].evidenceLayer !== "trusted-external") throw new Error("External evidence ingestion failed.");
